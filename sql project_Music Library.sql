...
-- select a.artist_id,a.name,count(t.track_id) as count
-- from artist a
...
-- ORDER BY milliseconds DESC;
 ...
 --    select name, milliseconds
	-- from track
	-- --where milliseconds> avg(milliseconds)  --aggregate functions are not allowed in WHERE
...
-- 		limit 1
-- 		)
-- select c.customer_id,c.first_name,c.last_name, cte.name as artist_name, sum(il.unit_price*il.quantity) as total_spent
-- 	from customer c 
...
-- with cte as
-- (
-- select c.country,g.name as top_genre,g.genre_id, count(il.quantity) as purchase,
...
-- join track t on t.track_id=il.track_id
-- join genre g on t.genre_id=g.genre_id
-- group by c.country,top_genre,g.genre_id
...
-- row_number() over(partition by c.country order by sum(total) desc) as rnk
-- from customer c
-- join invoice i on c.customer_id=i.customer_id
-- group by 1,2,3
